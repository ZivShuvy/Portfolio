...
function resetGameData() {
  ...
  elSpan.innerText =
    'Place ' + (gLevel.MINES - gCustomizeMode.minesPlaced) + ' more mines';
...
function expandShown(board, elCell, pos) {
  ...
  if (!hasMinesNegs(currCell.minesAroundCount))
    for (var neighborLocation of getNeighborsLocations(board, pos))
      expandShown(
        board,
        getElementByPosition(neighborLocation.i, neighborLocation.j),
        neighborLocation
      );
}
...
function hasMinesNegs(negsAmount) {
  return negsAmount > 0;
}
...
function onHintClicked(elHint) {
  ...
  if (gIsSoundOn) {
    var hintSound = gHints.isOn
      ? new Audio('sound/light-on.mp3')
      : new Audio('sound/light-on.mp3');
    hintSound.play();
  }
